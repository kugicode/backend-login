...
// Description: Authenticates a user and provides a login confirmation.
// Expects a JSON body: { "username": "testuser", "password": "securepassword" }
app.post('/login', async (req, res) => {
    console.log('POST /login request received. Body:', req.body);

    const { username, password } = req.body; // Destructure username and password from request body

    // 1. Basic validation for input
    if (!username || !password) {
        return res.status(400).json({ message: "Username and password are required." });
    }

    try {
        // 2. Find the user by username in the database
        const user = await usersCollection.findOne({ username: username });

        // 3. Check if user exists
        if (user === null) { // If user is null, no user with that username was found
            // Use 401 Unauthorized for invalid credentials (don't specify if username or password is wrong for security)
